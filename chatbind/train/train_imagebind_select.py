...
from chatbind.train.llava_trainer import LLaVATrainer

from chatbind import conversation as conversation_lib
...
class ModelArguments:
    ...
    repeat_feature_dim: int = field(default=-1)
...
def safe_save_model_for_hf_trainer_new(trainer: transformers.Trainer, output_dir: str):
    """Collects the state dict and dump to disk."""
    # state_dict = trainer.model.state_dict()
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if 'coco/train2014' in image_folder:
            ...
            if self.multimodal_cfg['repeat_feature_dim'] == -1:
                cur_token_len = 1
            else:
                cur_token_len = self.multimodal_cfg['repeat_feature_dim']
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = dataset_cls(tokenizer=tokenizer,
                                ...
                                multimodal_cfg=dict(
                                    ...
                                    repeat_feature_dim=getattr(data_args, 'repeat_feature_dim', -1)))
...
def train():
    ...
    if model_args.vision_tower is not None:
        model = LlavaLlamaImageBindSelectForCausalLM.from_pretrained(
    ...
    if model_args.vision_tower is not None:
        model_vision_dict = model.get_model().initialize_vision_modules(
            ...
            repeat_feature_dim=model_args.repeat_feature_dim,
        )
        dtype = torch.float32
        ...
        data_args.repeat_feature_dim = model_args.repeat_feature_dim
        data_args.is_multimodal = True

        model.config.tune_mm_mlp_adapter = training_args.tune_mm_mlp_adapter = model_args.tune_mm_mlp_adapter
        if model_args.tune_mm_mlp_adapter:
            print('tune mm mlp adapter')
    ...
    if training_args.fsdp is not None and len(training_args.fsdp) > 0:
        safe_save_model_for_hf_trainer_new(trainer=trainer,
